...
var isSubfolder =
  window.location.pathname.includes("/Seni/") ||
  window.location.pathname.includes("/Anggota/");
var dataPath = isSubfolder ? "../data/" : "data/";
var JSON_SENI_URL = dataPath + "seni.json";
var JSON_ANGGOTA_URL = dataPath + "anggota.json";
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  hydrateFormAnggota();
  wireValidation();
  markActiveNav(); // Fungsi yang diperbaiki
});

// ============ INISIALISASI DATA DARI JSON ============
async function initApp() {
  var seniData = localStorage.getItem(STORAGE_SENI);
  if (!seniData || JSON.parse(seniData).length === 0) {
    try {
      var response = await fetch(JSON_SENI_URL);
      if (response.ok) {
        var data = await response.json();
        setData(STORAGE_SENI, data);
      }
    } catch (error) {
      console.error("Gagal memuat data seni:", error);
    }
  }

  var anggotaData = localStorage.getItem(STORAGE_ANGGOTA);
  if (!anggotaData || JSON.parse(anggotaData).length === 0) {
    try {
      var response = await fetch(JSON_ANGGOTA_URL);
      if (response.ok) {
        var data = await response.json();
...
// ============ SORT ============
function urutkanSeni() {
  renderTabelSeni();
}
function urutkanAnggota() {
  renderTabelAnggota();
}
...
function sortData(data, sortKey) {
  ...
  if (sortKey === "nama_asc")
    sorted.sort(function (a, b) {
      return (a.nama_divisi || a.nama).localeCompare(b.nama_divisi || b.nama);
    });
  else if (sortKey === "nama_desc")
    sorted.sort(function (a, b) {
      return (b.nama_divisi || b.nama).localeCompare(a.nama_divisi || a.nama);
    });
  else if (sortKey === "terbaru")
    sorted.sort(function (a, b) {
      return b.id - a.id;
    });
  else if (sortKey === "terlama")
    sorted.sort(function (a, b) {
      return a.id - b.id;
    });
  return sorted;
...
function renderTabelSeni() {
  ...
  var sortKey = document.getElementById("sort-seni")
    ? document.getElementById("sort-seni").value
    : "terbaru";
  ...
  tbody.innerHTML = data
    .map(function (item) {
      ...
      return (
        "<tr data-row" +
        (isNew ? ' style="background-color: #e0f2fe;"' : "") +
        ">" +
        "<td>" +
        ...
        "</td>" +
        "<td>" +
        '<button type="button" class="btn-edit" onclick="location.href=\'tambah.html?edit=' +
        item.id +
        "'\">✏️ Ubah</button>" +
...
function renderTabelAnggota() {
  ...
  if (!table) return;
  var tbody = table.querySelector("tbody");
  var sortKey = document.getElementById("sort-anggota")
    ? document.getElementById("sort-anggota").value
    : "terbaru";
