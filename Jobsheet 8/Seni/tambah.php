...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($isEdit) {
            $stmt = $pdo->prepare("UPDATE seni SET no_divisi=:no_divisi, nama_divisi=:nama_divisi, keterangan=:keterangan, tahun=:tahun, updated_at=CURRENT_TIMESTAMP WHERE id=:id");
            $stmt->execute([
...
<div class="form-grid">
  ...
  <aside class="info-panel">
    ...
    <div class="info-card">
      ...
      <h3>Database</h3>
      <p class="info-stat">Total Divisi Tersimpan:</p>
      <p class="info-stat-value"><?= $pdo->query("SELECT COUNT(*) FROM seni")->fetchColumn() ?></p>
